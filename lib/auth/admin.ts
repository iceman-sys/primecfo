import { CONTACT_EMAIL } from '@/app/lib/pricing-plans';
 ...
 * falls back to the firm's CONTACT_EMAIL. Matching is case-insensitive.
 *
 * Always gate admin APIs and routes with `isAdminEmail` before returning data.
 */
export function getAdminEmails(): string[] {
  const raw = process.env.ADMIN_EMAILS?.trim();
  if (!raw) return [CONTACT_EMAIL.toLowerCase()];
  return raw
    .split(',')
