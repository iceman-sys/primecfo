...
/** Default best/worst revenue multipliers when history is too thin. */

export const DEFAULT_OPTIMISTIC_MULTIPLIER = 1.2;

export const DEFAULT_CONSERVATIVE_MULTIPLIER = 0.7;



/**

 * Derive scenario revenue multipliers from trailing monthly revenue volatility (coefficient of variation).

 * Clamped to ±10%–±35% so bands stay realistic for SMB cash planning.

 */

export function deriveScenarioMultipliers(revenues: number[]): {
  ...
  const samples = revenues.filter((r) => Number.isFinite(r) && r > 0);

  if (samples.length < 3) {

    return {

      optimistic: DEFAULT_OPTIMISTIC_MULTIPLIER,
  ...
  const variance =
    ...
    samples.reduce((sum, r) => sum + (r - mean) ** 2, 0) / Math.max(samples.length - 1, 1);

  const stdDev = Math.sqrt(variance);

  const cv = stdDev / mean;

  const band = Math.min(0.35, Math.max(0.1, cv * 1.25));
  ...
  return {
    ...
    conservative: Math.max(0.5, 1 - band * 1.5),

    volatilityPct: Math.round(cv * 1000) / 10,
...
export function computeCashForecast(
  ...
  const scenarioBands = deriveScenarioMultipliers(inputs.revenues);

  const optimisticParams: ScenarioParams = {

    revenueMultiplier: scenarioBands.optimistic,

  };

  const conservativeParams: ScenarioParams = {

    revenueMultiplier: scenarioBands.conservative,
  ...
  const optimistic = projectBalance(inputs, optimisticParams, horizon);

  const conservative = projectBalance(inputs, conservativeParams, horizon);
  ...
  return {
    ...
    components: {
      ...
      scenarioVolatilityPct: scenarioBands.volatilityPct,

      scenarioUsedDefaults: scenarioBands.usedDefaults,

      scenarioOptimisticMultiplier: scenarioBands.optimistic,

      scenarioConservativeMultiplier: scenarioBands.conservative,
