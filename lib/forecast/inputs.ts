...
import { buildMonthlyCashPoints } from '@/lib/forecast/parseCashFlowForForecast';
import {
  selectReconciledFullMonths,
  type MonthlyCashPoint,
} from '@/lib/forecast/scenarioBands';
...
export type ForecastInputs = {
  ...
  /** Last bank reconciliation date (capped to today). */
  lastReconciledDate: Date | null;
  /** Last 6 full reconciled months of net cash (+ owner draws) for scenario bands. */
  reconciledMonthlyCash: MonthlyCashPoint[];
};
...
export async function loadForecastInputs(
  ...
  const months = Math.max(caps.pnlHistoryMonths, 8);
  ...
  const [bankBalance, openInvoices, openBills, pnlAccrual, pnlCash, arRaw, cashFlowLive, lastReconciledRaw] =
    await Promise.all([
  ...
  const lastReconciledDate = capReconciliationDate(lastReconciledRaw, today);
  ...
  // Prefer a wide multi-month CF for scenario history (live pull or synced).
  const cfForScenarios = cashFlowMonthly ?? integratedCf;
  const allPoints = cfForScenarios ? buildMonthlyCashPoints(cfForScenarios, lastReconciledDate) : [];
  const reconciledMonthlyCash = selectReconciledFullMonths(allPoints, lastReconciledDate, 6);
